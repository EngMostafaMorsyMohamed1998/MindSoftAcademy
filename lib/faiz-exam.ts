...
export const FAIZ_OBJECTIVES: ObjectiveQuestion[] = [
  ...
  mcq("faiz-17", "جدار الحماية يعمل على:", "A firewall works by:", ["فلترة حركة الشبكة حسب قواعد مسموحة وممنوعة", "طباعة الملزمة", "شحن البطارية", "تصحيح الإملاء"], ["Filtering network traffic by allow/deny rules", "Printing the booklet", "Charging the battery", "Correcting spelling"], 0),
  ...
  mcq("faiz-19", "التصيد يعتمد على:", "Phishing relies on:", ["خداع المستخدم ليفتح رابطًا أو يعطي بياناته", "تحديث رسمي من الوزارة فقط", "كسر الباب الحديدي", "عطل المروحة"], ["Tricking the user into opening a link or giving data", "An official ministry update only", "Breaking an iron door", "A fan fault"], 0),
  ...
  mcq("faiz-23", "تطبيق الويب يتكون أساسًا من:", "A web application is mainly made of:", ["واجهة أمامية + خادم/خلفية + بيانات", "طابعة فقط", "كابل شاشة فقط", "سبورة ذكية بلا متصفح"], ["A frontend + a server/backend + data", "A printer only", "An HDMI cable", "A smart board with no browser"], 0),
  ...
  mcq("faiz-25", "بروتوكول الويب هو:", "HTTP is:", ["بروتوكول تواصل بين المتصفح والخادم", "لغة تلوين فقط", "نوع بطارية", "اسم طابعة"], ["A communication protocol between browser and server", "A colouring language only", "A battery type", "A printer name"], 0),
  mcq("faiz-26", "طلب الجلب يستخدم عادة لـ:", "A GET request is usually used to:", ["جلب صفحة أو بيانات بلا تغيير مقصود على الخادم", "حذف قاعدة البيانات", "إطفاء الراوتر", "طباعة الشهادة"], ["Fetch a page or data without intending to change the server", "Delete the database", "Turn the router off", "Print the certificate"], 0),
  mcq("faiz-27", "طلب الإرسال يستخدم عادة لـ:", "A POST request is usually used to:", ["إرسال بيانات ليُنشئ الخادم شيئًا أو يعالجه", "قراءة الوقت فقط", "تغيير لون الكابل", "قياس الحرارة"], ["Send data so the server can create or process something", "Read the time only", "Change the cable colour", "Measure temperature"], 0),
  mcq("faiz-28", "هيكل الصفحة يصف:", "HTML describes:", ["بنية الصفحة: عناوين وفقرات وروابط", "سرعة المعالج", "حجم الذاكرة", "حرارة الغرفة"], ["The page structure: headings, paragraphs, and links", "CPU speed", "RAM size", "Room temperature"], 0),
  mcq("faiz-29", "تنسيق الصفحة يتحكم في:", "CSS controls:", ["شكل الصفحة: الألوان والخطوط والتنسيق", "كلمة مرور الراوتر", "سعة البطارية", "اسم الطالب في الكشف"], ["The page look: colours, fonts, and layout", "The router password", "Battery capacity", "The student name on the register"], 0),
  mcq("faiz-30", "لغة التفاعل في الواجهة الأمامية تجعل الصفحة:", "Frontend JavaScript makes the page:", ["تتفاعل بعد التحميل: قوائم، تحقق، تحديث جزء منها", "كتابًا مطبوعًا فقط", "كبل شبكة", "امتحانًا ورقيًا"], ["Interactive after load: menus, checks, partial updates", "A printed book only", "A network cable", "A paper exam"], 0),
  mcq("faiz-31", "الواجهة البرمجية تفيد في:", "An API is useful for:", ["طلب بيانات أو خدمة من نظام آخر بطريقة متفق عليها", "طلاء الحائط", "قص الورق", "شحن القلم"], ["Requesting data or a service from another system in an agreed way", "Painting the wall", "Cutting paper", "Charging the pen"], 0),
  ...
  mcq("faiz-33", "ملف الصورة المضغوطة مناسب غالبًا لـ:", "A JPEG image is usually suitable for:", ["صور فوتوغرافية بتفاصيل كثيرة", "شعار يحتاج شفافية حادة", "ملف صوتي", "جدول درجات"], ["Photographs with lots of detail", "A logo that needs sharp transparency", "An audio file", "A grade table"], 0),
  mcq("faiz-34", "الصورة الشفافة مفيدة عندما:", "PNG is useful when:", ["تحتاج شفافية وحواف أوضح للرسوم", "تريد أغنية", "تريد فيديو طويل فقط", "تريد كلمة مرور"], ["You need transparency and clearer edges for graphics", "You want a song", "You want a long video only", "You want a password"], 0),
  mcq("faiz-35", "تجربة المستخدم تهتم بـ:", "User experience (UX) cares about:", ["سهولة الوصول للمطلوب ووضوح الخطوات وراحة الاستخدام", "لون السبورة في المعمل فقط", "سعر الكابل", "ارتفاع السقف"], ["How easily the user reaches the goal, how clear the steps are, and how comfortable it feels", "Only the lab board colour", "Cable price", "Ceiling height"], 0),
  ...
  mcq("faiz-40", "وسائط الفيديو تحتاج غالبًا إلى:", "Video media often needs:", ["ضغط مناسب حتى لا يثقل التحميل مع بقاء الجودة مقبولة", "تحويلها إلى كلمة مرور", "طباعتها على ورق فقط", "حذف الصوت دائمًا"], ["Sensible compression so loading is not heavy while quality stays acceptable", "Turning it into a password", "Printing it on A4 only", "Always deleting the audio"], 0),
  ...
  mcq("faiz-47", "بروتوكول الويب الآمن يختلف عن بروتوكول الويب بأنه:", "HTTPS differs from HTTP in that it:", ["يشفر الاتصال بين المتصفح والخادم", "يمنع فتح أي موقع", "يحذف الصور", "يغيّر لغة الصفحة إجباريًا"], ["Encrypts the connection between browser and server", "Blocks every site", "Deletes images", "Forces a language change"], 0),
  ...
  mcq("faiz-51", "الصورة المزيفة خطرها أنها:", "A deepfake is dangerous because it:", ["تبدو حقيقية وقد تُستخدم لخداع الناس", "تطيل عمر البطارية", "تحسن خط الطالب", "تغلق الواي فاي"], ["Looks real and may be used to deceive people", "Extends battery life", "Improves the student’s handwriting", "Turns Wi-Fi off"], 0),
...
export const FAIZ_ESSAYS: EssayQuestion[] = [
  essay(
    ...
    "المصطلح: التعلم الآلي والتعلم العميق.\nالسبب: التعلم الآلي يتعلم نمطاً من أمثلة. التعلم العميق يستخدم طبقات كتير وبيانات أكبر. من غير مراجعة ممكن يهلوس أو يتحيز أو يكشف بيانات.\nالمثال: فلتر بريد المدرسة تعلم آلي. تمييز صوت الطابور بطبقات كتير تعلم عميق.",
    "Term: machine learning and deep learning.\nReason: machine learning learns a pattern from examples. Deep learning uses many layers and more data. With no review it can hallucinate, show bias, or leak data.\nExample: the school spam filter is machine learning. Recognising the class name at assembly with many layers is deep learning.",
  ),
  essay(
    "faiz-e2",
    ...
    "المصطلح: التصيد.\nالسبب: الرسالة المزيفة بتسرق الحساب. أول خطوتين: متفتحش الرابط، وغّير كلمة المرور وبلّغ المدرس. إعادة إرسال الرابط «للتحذير» تنشر الخطر.\nالمثال: جروب الصف يوصله لينك درجات مزيف.",
    "Term: phishing.\nReason: the fake message steals the account. Two first steps: do not open the link, then change the password and tell the teacher. Forwarding the link as a warning spreads the risk.\nExample: the class group receives a fake marks link.",
  ),
  essay(
    "faiz-e3",
    ...
    "المصطلح: طبقات التطبيق.\nالسبب: المتصفح يرسل الطلب. الخادم يتحقق من الصلاحية. القاعدة تقرأ أو تحفظ. الرد يرجع. الصلاحيات على الخادم لأن الواجهة مكشوفة.\nالمثال: حفظ الحضور: الشاشة تجمع، الخادم يقرر، القاعدة تخزن.",
    "Term: application layers.\nReason: the browser sends the request. The server checks the right. The database reads or stores. The reply returns. Rights live on the server because the frontend is public.\nExample: save attendance: the screen collects, the server decides, the database stores.",
  ),
  essay(
    "faiz-e4",
    ...
    "المصطلح: دورة التحسين.\nالسبب: قيس مهمة واحدة، شوف ملاحظة الطلاب، غيّر حاجة واحدة، وقيس تاني.\nالمثال: صفحة حجز الحصة؛ المهمة إكمال الحجز؛ الزر مش واضح؛ سمّيه أوضح وأعد القياس.",
    "Term: improvement loop.\nReason: measure one task, note what students do, change one thing, then measure again.\nExample: a class-booking page; the task is finishing a booking; the button is unclear; rename it and measure again.",
  ),
];
